...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter
{
    ...
    protected void configure(final HttpSecurity http) throws Exception {
        			...
        			http.authorizeRequests().antMatchers(new String[] { "/login" }).access("isAnonymous()")
        					                .antMatchers(new String[] { "/**" }).access("hasRole('USER') or hasRole('ADMIN') or hasRole('DBA')")
        			   .and()
        				.formLogin().loginPage("/login").loginProcessingUrl("/login")
                                           .usernameParameter("lg").passwordParameter("ps")
                                           .successHandler(this.getAuthenticationSuccessHandler())
                       .and()
                        .rememberMe().rememberMeParameter("rm").rememberMeCookieName("krm")
                                  .tokenRepository(this.tokenRepository).tokenValiditySeconds(604800)
                       .and()
                        .sessionManagement()
